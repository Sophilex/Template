...
第一个递推式子显然是一个卷积形式，我们记其生成函数为$F$,不难得到$F^2*x+1=F$,故$F=\frac{1-\sqrt{1-4x}}{2x}$(另一个解推一下就知道不对)
...
* 任意两个费马数互质
  ...
  则$f_m=2^{2^m}+1=f_0f_1...f_nf_{n+1}...f_{m-1}+2$

  故$f_m\equiv 2(mod\ f_n)$

  故$(f_m,f_n)=(kf_n+2,f_m)=(f_n,2)$

  又$\forall n\in \mathbb{N},f_n\equiv 1(mod\ 2)$

  故$(f_m,f_n)=(f_n,2)=1$           $\square$

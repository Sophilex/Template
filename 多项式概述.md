...
我们考虑将n个有标号的元素分为k个**非空**无序集合的方案数，记其EGF为$F_{k}$,再考虑$f_i$表示在这个我们定义的集合中对集合元素的计数方式（也就是考虑元素在集合内的**排列方式**的个数，这是一个只跟集合大小有关的值），那么根据生成函数的定义，我们不难得到下式
...
## 关于处理$\prod (\frac{1}{1-x^i})^{c_i}$

首先取ln再exp显然比较合理

$ans=exp(\sum c_iln(\frac{1}{1-x^i}))$

我们考虑处理$\ln(\frac{1}{1-x^{p}})$

先考虑$A(x)=\frac{1}{1-x}=\{1,1,1,.....\}$，对其求导，得到$A'(x)=\frac{1}{(1-x)^2}=\{1,2,3,4,....\}$

那么$\frac{A'(x)}{A(x)}=\frac{1}{(1-x)^2}(1-x)=A(x)=\{1,1,....\}$

再做个积分，$\int\frac{A'(x)}{A(x)}=\ln(A(x))=\{0,1,\frac{1}{2},\frac{1}{3},...\}$，也就是$\ln\frac{1}{1-x}=\sum_{i=1}\frac{x^i}{i}$

现在我们再把$x^p$带入，就能得到$\ln\frac{1}{1-x^p}=\sum_{i=1}\frac{x^{ip}}{i}$

如果我们只要求前n项系数的话，这个式子只有$\frac{n}{p}$级别的有效项

对每一个p,也就是$c_i$暴力求系数并合并是$O(\sum \frac{n}{i})=O(nlogn)$的,最后再做一遍exp运算即可



## 关于处理$\prod_{i=1}^{n}(a_i-x)$

不难发现展开之后实际上只会有$n+1$项，我们可以直接设$dp_{i,j}$表示前i个式子相乘之后$x^j$的系数

$dp_{i,j+1}+=-dp_{i-1,j},dp_{i,j}+=dp_{i-1,j}*a[i]$
